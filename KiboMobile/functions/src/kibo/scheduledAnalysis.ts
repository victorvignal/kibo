 ...
 */
async function analyzeAllActiveUsers(
  batchSize: number = 50
): Promise<{
  analyzed: number;
  failed: number;
  ...
  for (const patientDoc of patientsSnapshot.docs) {
    ...
    if (patientData.status === "inactive") continue;

    try {
      const profile = await analyzeUserData(patientId, 30);
...
async function createRiskAlert(
  ...
  const severity =
    profile.chatContext.riskLevel === "critical"
      ? "high"
 ...
 */
async function runWeeklyDeepAnalysis(): Promise<{
  analyzed: number;
  failed: number;
  newPatterns: number;
}> {
  ...
  for (const patientDoc of patientsSnapshot.docs) {
    ...
    try {
      ...
      const profile = await analyzeUserData(patientId, 90);
 ...
 */
export const scheduledAnalysis6h = functions.pubsub
 ...
 */
export const scheduledAnalysisDaily = functions.pubsub
 ...
 */
export const scheduledWeeklyDeep = functions.pubsub
 ...
 */
export const triggerAnalysis = functions.https.onRequest(async (req: Request, res: Response) => {
  // Only allow POST
  if (req.method !== "POST") {
  ...
  try {
    const profile = await analyzeUserData(patientId, days || 30);
