 ...
 */
function detectDayPatterns(records) {
    ...
    const dayAverages = dayMoodSum.map((sum, i) => dayCount[i] > 0 ? sum / dayCount[i] : 5 // default to neutral
    );
 ...
 */
function detectTimePatterns(records) {
    const timeMoodSum = {
        morning: 0,
        afternoon: 0,
        evening: 0,
 ...
 */
function detectTriggers(records) {
    const triggers = [];
    // Sort by date
 ...
 */
function detectTrends(records, days = 14) {
    if (records.length < 4) {
        return {
    ...
    const avgRecent = (arr, field) => arr.length > 0 ? arr.reduce((acc, r) => acc + (Number(r[field]) || 0), 0) / arr.length : 5;
    const avgOlder = (arr, field) => arr.length > 0 ? arr.reduce((acc, r) => acc + (Number(r[field]) || 0), 0) / arr.length : 5;
 ...
 */
function detectPatterns(records, days = 30) {
    if (records.length === 0) {
        return {
            patterns: {
                bestDays: [],
